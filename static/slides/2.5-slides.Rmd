...
### [The Firm's Short-Run Supply Decision](#54)
...
$$AR(q)=\frac{R}{q}$$
  - $AR(q)$ is **always** equal to the price! Why?
...
$$MR(q) = \frac{\Delta R(q)}{\Delta q} \approx \frac{R_2-R_1}{q_2-q_1}$$
  ...
  - .hi-purple[For a .ul[*competitive*] firm (only), `\\(MR(q) = p\\)`, the price!]
...
ggplot(data.frame(x=c(0,10)), aes(x=x))+
  stat_function(fun=mr, geom="line", size=2, color = "blue")+
    geom_label(aes(x=7,y=mr(7)), color = "blue", label="MR(q)=AR(q)=10", size = 6)+
    scale_x_continuous(breaks=seq(0,10,1),
                     limits=c(0,10),
                     expand=expand_scale(mult=c(0,0.1)))+
...
marginal_plot+
    geom_label(aes(x=8,y=mc(8)), color = "red", label="Supply", size = 7)
...
.pull-right[
.center[
![:scale 100%](../images/shutitdown.jpg)
]
]
...
supply_plot+
  ...
  geom_label(aes(x=5.5,y=price4(5.5)), color = "blue", label=expression(MR(q)), size = 3)+
  geom_hline(yintercept=4, size=1, linetype="dashed")+
  geom_label(x=6,y=4, label="Min AVC(q)", size =5, color="brown")+
  stat_function(fun=marginal, xlim=c(2,10), geom="line", size=3.5, color = "red")+
    geom_label(aes(x=3,y=marginal(3)), color = "red", label="Supply", size = 5)
...
.pull-right[
.center[
Firm's .red[short run (inverse) supply]:
]
$$\begin{cases}
