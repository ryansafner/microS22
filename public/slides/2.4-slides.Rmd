...
### [Costs in the Short Run](#21)
### [Costs in the Long Run](#43)
...
- Remember, .hi[economic costs] are broader thanthe common conception of “cost”
...
- This leads to the difference between
  ...
  - .hi[Economic profit]: revenues minus accounting & *opportunity* costs
...
- Economic importance (later): Break-even price & shut-down price
]
]
.pull-right[
```{r, fig.retina=3}
...
- .hi-purple[Long run average cost (LRAC)] curve "envelopes" the lowest (optimal) parts of all the SRAC curves!
...
ggplot(data.frame(x=c(0,10)), aes(x=x))+
  ...
  annotate("segment", x = 5, xend = 115, y = 10000, yend = 10000, color = "green", size=2, alpha=0.9, arrow=arrow(length=unit(0.5,"cm"), ends="last", type="closed"))+
    geom_label(x=62.5, y=10000, size=4, label="Economies of Scale", color="green")+

  annotate("segment", x = 135, xend = 195, y = 10000, yend = 10000, color = "red", size=2, alpha=0.9, arrow=arrow(length=unit(0.5,"cm"), ends="last", type="closed"))+
  geom_label(x=170, y=10000, size=4, label="Diseconomies of Scale", color="red")+
  
  geom_label(x=125, y=10000, size=4, label="M.E.S.", color="black")+

   scale_x_continuous(breaks=c(50,100,125,150,200),
                     limits=c(0,200),
                     expand=expand_scale(mult=c(0,0.1)))+
  scale_y_continuous(breaks=c(0,10000,16375,20000,30000,40000,50000),
                     limits=c(0,50000),
                     expand=expand_scale(mult=c(0,0.1)),
                     labels = scales::dollar)+
  #scale_colour_manual("Curves", values = line_colors, guide=F)+
